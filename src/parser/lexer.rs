...
pub fn parse_string(input: &str) -> IResult<&str, String> {
    alt((
        delimited(char('\''), map(many0(none_of("'")), |chars: Vec<char>| chars.into_iter().collect()), char('\'')),
        delimited(char('"'), map(many0(none_of("\"")), |chars: Vec<char>| chars.into_iter().collect()), char('"')),
    ))(input)
}

pub fn parse_integer(input: &str) -> IResult<&str, i64> {
    map(recognize(pair(opt(char('-')), digit1)), |s: &str| s.parse().unwrap())(input)
}

pub fn parse_float(input: &str) -> IResult<&str, f64> {
    map(recognize(tuple((opt(char('-')), digit1, char('.'), digit1))), |s: &str| s.parse().unwrap())(input)
}

pub fn parse_binary(input: &str) -> IResult<&str, i64> {
    map(preceded(tag("0b"), take_while1(|c| c == '0' || c == '1')), |s: &str| i64::from_str_radix(s, 2).unwrap())(input)
}

pub fn parse_hexadecimal(input: &str) -> IResult<&str, i64> {
    map(preceded(tag("0x"), take_while1(|c: char| c.is_digit(16))), |s: &str| i64::from_str_radix(s, 16).unwrap())(input)
}
