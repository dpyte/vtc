...
mod tests {
	...
	fn test_std_if_intrinsic() {
		...
		runtime.load_vtc(vtc_config).expect("Failed to load VTC configuration");

		// Test true condition
		let true_result = runtime.get_string("test_if_else", "true_condition")
			.expect("Failed to get true_condition value");
		assert_eq!(true_result, "condition is true");

		// Test false condition
		let false_result = runtime.get_string("test_if_else", "false_condition")
			.expect("Failed to get false_condition value");
		assert_eq!(false_result, "condition is false");

		// Test numeric condition
		let numeric_result = runtime.get_string("test_if_else", "numeric_condition")
