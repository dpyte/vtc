...
mod tests {
	...
	fn test_add_value_to_existing_namespace() {
		...
		assert!(runtime
			.add_value("test_ns", "key1", Value::String(String::from("value1")))
			.is_ok());
	...
	fn test_add_value_to_non_existent_namespace() {
		let mut runtime = Runtime::new();
		assert!(runtime
			.add_value("new_ns", "key1", Value::String(String::from("value1")))
			.is_ok());
	...
	fn test_add_different_value_types() {
		let mut runtime = Runtime::new();
		assert!(runtime
			.add_value("ns", "string_key", Value::String(String::from("value1")))
			.is_ok());
		assert!(runtime
			.add_value("ns", "int_key", Value::Number(Number::Integer(42)))
			.is_ok());
		assert!(runtime
			.add_value("ns", "bool_key", Value::Boolean(true))
			.is_ok());
	}
	...
	fn test_update_existing_value() {
		let mut runtime = Runtime::new();
		runtime
			.add_value("ns", "key", Value::String(String::from("")))
			.unwrap();
		assert!(runtime
			.add_value("ns", "key", Value::String(String::from("new")))
			.is_ok());
	}
	...
	fn test_remove_existing_value() {
		let mut runtime = Runtime::new();
		runtime
			.add_value("ns", "key", Value::String(String::from("value")))
			.unwrap();
